            ...
            {activeTab === 'api' && (
              ...
              {activeProviderUsesApiUrl && (
                <label className="block">
                  ...
                  <div data-selectable-text className="mt-1.5 min-h-[22px] flex items-center text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyEnabled ? (
                      <span className="text-yellow-600 dark:text-yellow-500">已开启代理，请求会通过同源代理转发到当前 API URL。</span>
              ...
              {apiProxyAvailable && activeProfile.provider === 'openai' && (
                <div className="block">
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyLocked ? '当前部署已锁定 API 代理为开启，请求会通过同源代理转发到当前 API URL。' : '当前部署提供同源代理时可手动开启。开启后用于解决浏览器跨域限制，并按当前配置的 API URL 转发请求。'}
                  </div>
                </div>
              )}
