...
export function buildApiUrl(
  ...
  if (useApiProxy) {
    const proxyBaseUrl = encodeURIComponent(normalizedBaseUrl || proxyConfig?.target || '')
    return `${proxyConfig?.prefix ?? DEFAULT_PROXY_PREFIX}/${proxyBaseUrl}/${apiPath}`
  }

  return normalizedBaseUrl ? `${normalizedBaseUrl}/${apiPath}` : `/${apiPath}`
}

export function buildDynamicProxyTarget(requestUrl: string, prefix = DEFAULT_PROXY_PREFIX): string | null {
  ...
  const encodedBaseUrl = rest.slice(0, slashIndex)
  const apiPath = rest.slice(slashIndex + 1)
  if (!apiPath) return null

  try {
    const baseUrl = new URL(decodeURIComponent(encodedBaseUrl))
...
export function isApiProxyAvailable(proxyConfig: DevProxyConfig | null = readClientDevProxyConfig()): boolean {
  const runtimeValue = readRuntimeEnv(import.meta.env.VITE_API_PROXY_AVAILABLE)
